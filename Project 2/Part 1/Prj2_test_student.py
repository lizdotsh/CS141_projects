...
'608', '715', '920', '307']
PREFIXES = ['958','959','555'] + [str(i)+'11' for i in range(10)]

def test_program(filename,ret_val = None, sides = None):
  ...
  print('Testing:')
  try:
    phone = fake_stdout.getvalue().rstrip()
    area = phone[0:3]
    prefix = phone[4:7]
    suffix = phone[8:]
    print('Output:\n',phone, sep='')
    print('Checking area code')
    print('Area code:', area)
    assert area in AREA_CODES
    ...
    assert area.isdigit() == True
    assert prefix.isdigit() == True
    assert suffix.isdigit() == True
    print('success!\n')
...
#Test reading
print('Testing generate_phone.py \n')
try:
    test_program('generate_phone.py')
